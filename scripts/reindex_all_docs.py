#!/usr/bin/env python3
"""Re-index all existing doc versions into doc_chunks.

Usage:
    docker compose exec web python scripts/reindex_all_docs.py
    docker compose exec web python scripts/reindex_all_docs.py --purge-old

Options:
    --purge-old    Delete all rows from doc_chunks first, then reindex.
...
from app.db.rag_models import DocChunk  # noqa: E402
from app.db.doc_models import (  # noqa: E402
    AppDocVersion,
    GlobalDocVersion,
    RepoDocVersion,
)
from app.doc.service import make_default_doc_service  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Reindex all doc versions into doc_chunks.")
    parser.add_argument(
        "--purge-old",
        action="store_true",
        help="Delete all existing doc_chunks rows before reindexing (one-time cleanup).",
    )
    ...
    if args.purge_old:
        deleted = db.execute(delete(DocChunk)).rowcount
        db.commit()
        print(f"[purge-old] Deleted {deleted} rows from doc_chunks")

    svc = make_default_doc_service(db)
    total = 0
    errors = 0

    # Global docs
    global_rows = _latest_versions(db, GlobalDocVersion, ["section_name"])
    for row in global_rows:
        try:
            result = svc.index_doc(
    ...
    # App docs
    app_rows = _latest_versions(db, AppDocVersion, ["app_name", "section_name"])
    for row in app_rows:
        try:
            result = svc.index_doc(
                "app", row.id, row.body,
    ...
    # Repo docs
    repo_rows = _latest_versions(db, RepoDocVersion, ["pattern", "section_name"])
