...
def app_docs_cards(
    ...
    domain_filter = domain.strip() or None
    names = _sort_app_names(vw.list_distinct_apps_with_docs(db, domain=domain_filter))
    if q.strip():
...
def repo_docs_cards(
    ...
    domain_filter = domain.strip() or None
    patterns = _sort_repo_patterns(vw.list_distinct_repo_patterns_with_docs(db, domain=domain_filter))
    if q.strip():
        qn = q.strip().lower()
