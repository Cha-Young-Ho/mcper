...
def workflow_chunk_vector_ids(
    ...
    *,
    ...
    stmt = stmt.order_by(WorkflowChunk.embedding.cosine_distance(query_embedding)).limit(limit)
...
def workflow_chunk_fts_ids(
    ...
    *,
    ...
    stmt = (
        ...
        .where(WorkflowChunk.chunk_type != "parent")
        .where(
            text("workflow_chunks.content_tsv @@ plainto_tsquery('simple', :fts_q)").bindparams(
                fts_q=q
            )
        )
    )
...
def hybrid_workflow_search(
    ...
    *,
    ...
    if workflow_type_filter:
        count_stmt = count_stmt.where(WorkflowChunk.workflow_type == workflow_type_filter)
    if app_name:
    ...
    try:
        ...
        fts_ids = workflow_chunk_fts_ids(
            db, query=query, workflow_type=workflow_type_filter,
        ...
        v_ids = workflow_chunk_vector_ids(
            db, query_embedding=qvec, workflow_type=workflow_type_filter,
    ...
    if parent_ids:
        parents_by_id = {
            p.id: p
            for p in db.scalars(select(WorkflowChunk).where(WorkflowChunk.id.in_(parent_ids))).all()
        }
