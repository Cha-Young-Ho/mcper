...
def hybrid_rule_search(
    ...
    *,
    ...
    # ORDER BY CASE 로 ranked 순서를 DB 레벨에서 유지 (Python 재정렬 제거, P10).
    order_case = case({rid: idx for idx, rid in enumerate(ranked)}, value=RuleChunk.id)
    ordered = list(
        db.scalars(
            select(RuleChunk).where(RuleChunk.id.in_(ranked)).order_by(order_case)
        ).all()
    )
